...
- **권고 해소 방향**: (1) **PR #168 진행**: macOS CI runner 추가 (`.github/workflows/ci.yml` 에 `macos-poc` job — `continue-on-error: true` PoC) — 본 단계 (2) PoC 통과 시 `closed` 전환 (`continue-on-error` 제거 + required check 승격) (3) PoC 실패 시 fallback 도입 (`src/storage/Database.ts` try/catch + in-memory cosine 비-벡터 검색 모드)
