...
### G-022 [active] 사용자 마무리 의도 후 작업 진입 차단 (메타 학습)

- **규칙**: 사용자 발화에 **종결/지연/핸드오프/추궁** 4 분류 신호 표현이 박혀 있으면, 사용자 명시 선택 (PR 생성 / 세션 종료 / 다른 작업) 박힐 때까지 새 작업 진입 금지. codex 협의 권고는 진입 시점 path 권고일 뿐 — 진입 _여부_ 결정 자체는 사용자 명시 선택 후만 박힘. **허용 패턴 우선순위 (codex 019e5129 NEEDS_CHANGES #2 정합)**: 같은 발화 안에 진입 의사 명시 ("작업 진행해" / "PR 박아") 가 박혀있으면 검출 신호보다 허용 패턴이 우선.

**검출 표현 (사용자 발화 분석, 공백 정규화 후 매칭 — codex 019e5129 BLOCKING #2 정합)**:
- 매칭 전 사용자 발화는 **공백 정규화** (whitespace strip) 후 검출 — 무공백 variant ("다음세션", "핸드오프작성") 도 동일 매칭. 단순 grep regex 는 spacing variant 가 우회 path 박힘
- 종결 신호: "세션 종료" / "세션 마무리" / "오늘 마무리" / "마치자" / "마무리하자"
- 지연 신호: "다음 세션" / "다음세션" / "다음 세션에서" / "다음세션에서" / "내일"
- 핸드오프 신호: "핸드오프 작성" / "핸드오프작성" / "핸드오프 박아" / "state 갱신해"
- 추궁 신호: "오래걸리냐" / "왜 이렇게 오래" / "왜이렇게 오래" / "여태 ~ 안" / "검증시켰는데"
- 본 신호 표현 박히면 진입 의사 없음 으로 해석 → AskUserQuestion 으로 명시 선택 받음 (단, **허용 패턴 우선 적용 후**)

**금지 패턴**:
- "기본 권고로 진행" / "codex 권고 정합 진입" / "임의 기본 권고" 표현으로 진입 결정 우회
- 메모리/handoff §다음 진입점 박힘 ≠ 본 세션 진입 박음 권고. 다음 진입점 = "다음 세션에 박을 작업 목록"
- codex 협의 권고를 "진입 결정 자체" 우회 핑계로 사용 (codex 권고 = 진입 _후_ 작업 박는 path 권고)
- "사용자 결정 대기" 박힌 상태에서 임의 옵션 선택 후 진입 ("기본 권고 — option B" 같은 패턴)

**허용 패턴 (검출 신호보다 우선 — codex 019e5129 NEEDS_CHANGES #2 정합)**:
- 사용자 발화에 진입 의사 명시 ("작업 진행해" / "PR 박아" / "구현 시작" / "진행해") → 진입 박음 (검출 신호 동시 박혀있어도 진입 의사 우선)
- 사용자 발화에 codex 협의 위임 명시 ("코덱스랑 협의해서 진행해" / "codex 권고 받아서 박아") → codex 협의 + 1순위 권고 진입 박음
- 회색지대 명사적 지시 ("핸드오프 읽고", "state 확인하고") = 검출 신호 아님 (동사 페어 "핸드오프 작성/박아", "state 갱신해" 만 신호 — evaluator a4dc5ef9 권고 정합)
- autonomous_progress 정합 path: codex 협의 → 1순위 권고 진입 (사용자 위임 명시 후만)

- **Why**: Sprint 017 본 세션 (2026-05-24) G-021 정식화 박은 직후 본인이 같은 패턴 위반 — handoff #239 (M4 docs) 머지 후 G-021 (#240) + Schema v06 spec (#241) 임의 진입 → 사용자 본질 지적 "아니 다음세션에서 진행하려고 핸드오프작성하고 검증시켰는데 뭐이렇게 오래걸리냐?" → PR #241 close. **메타 위반 패턴** — 가드레일 박은 본 세션에서 그 가드레일 위반. G-021 (docs PR dual review) 과 결함 축이 다름 (G-022 = 진입 결정 자체 정합).

  G-022 가 결함 축으로 분리되어야 하는 이유 (codex 019e5119 권고):
  > "G-021은 'docs PR dual review'를 막는 가드레일이고, G-022는 '사용자 마무리 의도 이후 새 작업 진입 금지'라 결함 축이 다릅니다. Sprint 018 위임도 가능하지만, 이번 위반이 바로 G-021 직후 발생했으므로 지연하면 같은 실패를 한 번 더 허용하는 구조입니다."

- **검출 path**:
  - 사전 (자가 강제): 사용자 발화 분석 시 (1) 허용 패턴 (진입 의사 명시) 먼저 점검 — 박혀있으면 진입 박음 (2) 허용 패턴 없을 때만 검출 표현 4 분류 (공백 정규화 후) 매칭 → 박혀있으면 AskUserQuestion 으로 명시 선택 받기 전 작업 진입 금지
  - 사후 (회고): handoff §self-feedback 안에 위반 시점 + 위반 작업 + 사용자 정정 발화 명시 (학습 #18 확증 path 정합)
  - 자동 강제 후보 (mini-milestone β, hook — codex 019e5129 NEEDS_CHANGES #3/#5 정합): SessionStart hook 이 transcript history (`.specstory/history/` 또는 `~/.claude/projects/<project>/conversations/*.jsonl`) 의 마지막 user 발화 N건 추출 + 공백 정규화 + 검출 표현 매트릭스 매칭. data source 신뢰 path 박힘 = transcript 파일 직접 읽기 (Claude 가 발화 인식 생략/위조 불가). **mini-milestone β 구현 우선순위 (codex 019e5129 NEEDS_CHANGES #5 권고)**: (a) G-018 산출물 vs diff 자동 대조 → (b) G-021 dual review 증거 regex 검증 → (c) G-022 발화 정규화 + 차단 sentinel 박음 순서 — 방어 성격 강한 순.

  - **자동 강제 박힘 (Sprint 018 M0, advisory → blocking)**:
    - **T01 (PR #246, advisory)**: SessionStart hook 이 transcript 직접 읽어 최신 user 발화 검출 → advisory 출력 (non-blocking, exit 0). `.flowset/scripts/transcript-reader.mjs` + `.flowset/scripts/check-finalization-intent.mjs`.
    - **T02 (mini-milestone β PR C, blocking)**: 실 차단은 **PreToolUse hook** 이 강제 (`.flowset/hooks/pre-tool-use.mjs` + `.flowset/scripts/g022-tool-classifier.mjs`). **contract deviation 정합 (codex 019e634d)** — SessionStart 는 exit 2 차단 불가(Claude Code 공식 문서: SessionStart/Setup/Notification 은 exit 2 시 stderr 표시 후 진행). 따라서 contract §2 "session-start.mjs exit 2 blocking" 는 SessionStart advisory 유지 + PreToolUse enforcement 로 재해석.
    - **차단 범위 (false positive 0 — codex 019e634d §2)**: advisory === `BLOCK_ENTRY` 일 때만 활성. Write/Edit/MultiEdit + non-closeout 경로 → 차단 / 새 작업 커밋(feat·fix·refactor·perf) → 차단. **항상 통과 (closeout)**: state.md / handoffs/ / logs/ / eval-results/ / known-issues.md / 사용자 memory / docs·chore·style·ci·test·revert 커밋 / read-only Bash / gh pr·git push (closeout PR 보호). transcript 미발견·발화 추출 실패·검출 에러 = **fail-open** (exit 0).
    - **해제 (unblock — codex 019e634d §3)**: enforcement source of truth = 매 PreToolUse 호출 transcript 재독 (별도 state 파일 미사용). `transcript_path` 주어지면 그 파일만 신뢰 (stale 세션 fallback 금지 — false positive 회피). 사용자 entry 발화 ("PR 생성" / "계속 진행" / "다른 작업 진행" / "구현 시작") 또는 delegation ("codex 권고대로 진행") 박히면 advisory ALLOW_ENTRY → 게이트 즉시 inert.
    - **차단 셸 동작 (codex 019e634d 2차 review BLOCKING 반영)**: gh pr create / git push(non-dry-run) / 새 브랜치(git checkout -b·switch -c) / dependency 설치(npm·pnpm·yarn install/add) / 파일 쓰기 cmdlet(Set-Content·Add-Content·Out-File·Tee-Object·sed -i·perl -i, closeout 경로 미참조 시). NotebookEdit 도 파일 쓰기 tool 로 차단 범위 포함.
    - **범위 밖 (out-of-scope — 명시적 미차단, 운영상 Claude 가 closeout 에 쓰지 않는 경로)**: bare `>` / `>>` redirect (read-only inspection 캡처 false-positive 위험), `node -e` 파일 쓰기, cp/mv/Copy-Item/Move-Item, **MCP write tool (`mcp__...` PR 생성/파일 변경/issue mutation — PreToolUse matcher 범위 밖)**. 본 잔여는 자가 강제(사전 점검)로 보완 — 자동화는 차후 mini-milestone.

- **How to apply**:
  - 사용자 발화에 종결/지연/핸드오프/추궁 4 분류 신호 표현 박힘 점검 (공백 정규화 후) → 박혀있으면 (허용 패턴 동시 박혀있지 않은 경우) AskUserQuestion 으로 옵션 매트릭스 제시 + 사용자 명시 선택 후만 진입
  - "codex 협의 권고 받아서 진행" 명시 박힌 경우만 codex 1순위 권고 자율 진입 박음 (autonomous_progress 정합)
  - handoff §다음 진입점 박힘 만으로 진입 결정 박지 않음 — 사용자 명시 선택 후만 박음
  - 본 PR 자체 자기 적용 박음: PR body 에 사용자 위임 발화 ("코덱스랑 협의해서 진행해") 인용 + codex 1순위 권고 (G-022 정식화) 인용 + 진입 박힘 정합 명시

- **출처**: 본 세션 학습 #8 위반 4번째 회고 (2026-05-24, handoff 2026-05-24.md §8.3) + codex 019e5119 권고 ("1순위 진입점: G-022 정식화. G-022는 이번 세션의 직접 결함, 즉 '마무리 의도 후 임의 진입' 재발을 막는 메타 가드레일이라 선행하는 게 정합합니다. 작고 되돌리기 쉬우며, G-021 직후 같은 패턴을 위반했다는 점 때문에 우선순위가 높습니다.").
...
- 2026-05-24 (Sprint 017 M5, G-021 후속): **G-022 신규 등록** — 사용자 마무리 의도 후 작업 진입 차단 (메타 학습). 본 세션 학습 #8 위반 4번째 회고 — handoff #239 (M4 docs) 머지 후 G-021 (#240) + Schema v06 spec (#241) 임의 진입 → 사용자 본질 지적 "아니 다음세션에서 진행하려고 핸드오프작성하고 검증시켰는데 뭐이렇게 오래걸리냐?" → PR #241 close. **메타 위반 패턴** (G-021 박은 본 세션에서 같은 패턴 재반복). codex 019e5119 권고 ("1순위 진입점: G-022 정식화. 작고 되돌리기 쉬우며, G-021 직후 같은 패턴을 위반했다는 점 때문에 우선순위가 높습니다. G-021은 'docs PR dual review'를 막는 가드레일이고, G-022는 '사용자 마무리 의도 이후 새 작업 진입 금지'라 결함 축이 다릅니다."). 검출 표현 매트릭스 박음 — 종결/지연/핸드오프/추궁 신호 4 분류 + 금지/허용 패턴 명시. mini-milestone β hook 자동화 후보 명시.
- 2026-05-23 (Sprint 018 M0 T01, PR #246): **G-022 advisory 자동화** — SessionStart hook 이 transcript 직접 읽어 최신 user 발화 검출 + advisory 출력 (non-blocking). `transcript-reader.mjs` + `check-finalization-intent.mjs` 박음.
- 2026-05-26 (Sprint 018 M0 T02, mini-milestone β PR C): **G-022 blocking 전환** — advisory → 실 차단. enforcement 는 **PreToolUse hook** (`pre-tool-use.mjs` + `g022-tool-classifier.mjs`). **contract deviation 기록** (codex 019e634d): SessionStart 는 exit 2 차단 불가(Claude Code 공식 문서) → contract §2 "session-start.mjs exit 2 blocking" 는 SessionStart advisory 유지 + PreToolUse enforcement 로 재해석. 차단 범위 = BLOCK_ENTRY 시 non-closeout Write/Edit/MultiEdit + 새 작업 커밋(feat/fix/refactor/perf), closeout(state/handoffs/logs/memory) + docs·chore 커밋 + read-only + gh pr·push 통과, 에러 fail-open (false positive 0). entry phrase 보강 (PR 생성/계속 진행/다른 작업/codex 권고대로). 회귀 +46 (classifier 25 + hook 16 + intent 5). 단위 1777 → 1823.
