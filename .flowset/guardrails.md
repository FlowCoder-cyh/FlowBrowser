...
- **검출 path**:
  ...
  - **자동 강제 박힘 (Sprint 018 M0, advisory → blocking)**:
    ...
    - **해제 (unblock — codex 019e634d §3)**: enforcement source of truth = 매 PreToolUse 호출 transcript 재독 (별도 state 파일 미사용). 사용자 entry 발화 ("PR 생성" / "계속 진행" / "다른 작업 진행" / "구현 시작") 또는 delegation ("codex 권고대로 진행") 박히면 advisory ALLOW_ENTRY → 게이트 즉시 inert.
