...
- **규칙**: 사용자 발화에 종결/마무리/지연 의도 표현이 박혀 있으면, 사용자 명시 선택 (PR 생성 / 세션 종료 / 다른 작업) 박힐 때까지 새 작업 진입 금지. codex 협의 권고는 진입 시점 path 권고일 뿐 — 진입 _여부_ 결정 자체는 사용자 명시 선택 후만 박힘.

**검출 표현 (사용자 발화 분석)**:
- 종결 신호: "세션 종료" / "세션 마무리" / "오늘 마무리" / "마치자"
- 지연 신호: "다음 세션" / "다음 세션에서" / "다음 작업으로" / "내일"
- 핸드오프 신호: "핸드오프 작성" / "핸드오프 박아" / "state 갱신해"
- 추궁 신호: "오래걸리냐" / "왜 이렇게 오래" / "여태 ~ 안" / "검증시켰는데"
- 본 신호 표현 박히면 진입 의사 없음 으로 해석 → AskUserQuestion 으로 명시 선택 받음
...
**허용 패턴**:
- 사용자 발화에 진입 의사 명시 ("작업 진행해" / "PR 박아" / "구현 시작") → 진입 박음
...
- **검출 path**:
  - 사전 (자가 강제): 사용자 발화 분석 시 검출 표현 grep → 박혀있으면 AskUserQuestion 으로 명시 선택 받기 전 작업 진입 금지
  - 사후 (회고): handoff §self-feedback 안에 위반 시점 + 위반 작업 + 사용자 정정 발화 명시 (학습 #18 확증 path 정합)
  - 자동 강제 후보 (mini-milestone β, hook): SessionStart hook 의 최근 사용자 발화 분석 + 마무리 신호 표현 박힘 시 다음 진입점 매트릭스 자동 제시 (코드 진입 차단)

- **How to apply**:
  - 사용자 발화에 종결/마무리 신호 표현 박힘 점검 → 박혀있으면 AskUserQuestion 으로 옵션 매트릭스 제시 + 사용자 명시 선택 후만 진입
