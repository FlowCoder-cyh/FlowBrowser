...
- **Phase**: **3 진행 중 (Sprint 018 M2 T21 머지 완료 `0bb01fa` (#266) — 워크스페이스 공유 설계 시안, 본 state 동기화 docs PR 진행 중)** (**M0+M1+M2 사실상 종료 → M3 진입 예정. M2 = T17 라인(a~e+write-path) + T21 SharedWorkspaceFormat 설계 시안 전부 머지, T20 = Sprint 019 defer**). carryover **3 작업 잔여** (T20=Sprint 019 defer + T22~T26 + PRD v0.5.0). **codex 019e6e4d 우선순위 협의 — 순차 실행: ~~write-path wiring(#262)~~ → ~~T17e(#264)~~ → ~~T21(#266)~~ → T22/T09 → PRD v0.5.0. T20 = Sprint 019 defer(학습 데이터셋 부재).**
- **Sprint**: **018 M2 T21 머지 완료 (#266 `0bb01fa`) — 워크스페이스 공유 설계 시안(SharedWorkspaceFormat), 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **21** (#246~#266 feat/docs, 본 docs PR 머지 시 22 — `git log --first-parent main --grep=WI-S018` 실측). M0+M1+M2 종료 (**M2 = T17a~e + write-path wiring + T21 spike 전부 머지, T20 Sprint 019 defer**) → M3 진입 예정. 다음 = **T22/T09** (M3 — 단위 회귀 + 시나리오 cover). **T17 라인 종결(query/write/생성 UX 전부 provider-aware + E2E 격리) + 공유 포맷 설계 박힘**.
...
- **최근 갱신 (Sprint 018 M2 T21 머지 완료 — 워크스페이스 공유 설계 시안)**: 2026-05-29 (PR #266 T21 SharedWorkspaceFormat 설계 시안 머지 `0bb01fa` — CI 6/6 SUCCESS. 신규 `.flowset/specs/v05-collab-spike.md` (+318/-0/1 파일, spec only 코드 변경 0). 단방향 포터블 파일 공유(`.fbworkspace` = gzip envelope + 무결성 SHA-256 + 선택 Ed25519 TOFU 서명) 설계. **"공유 파일 = 공격자 입력"** 전제 — untrusted deep schema/limits validator(byte/count/length/depth cap + enum/finite int/FK 무결성 의미 검증) P0. 서명 = 신원 증명 아닌 tamper-evidence + TOFU 연속성(local-first 무PKI 정직 한정). 기존 `WorkspaceExportV1` v06 canonical payload + `ExportPrivacyGate` filtered cascade(Notion spec §13) 재사용. 실시간 협업 = Phase 4(scope 외, 출처 contract §2/§8). 구현 Sprint 021(S021-a~k). codex 019e6fcf 사전 설계 협의(BLOCKING 2/NC 5/NOTABLE 1) → 작성 → dual review evaluator **Pass 5/Partial 1/Fail 0**[agentId `ac2951b4aa4fb4e5b`, Partial 1=§2 "실시간=Phase 4" SSOT 출처 표기 → v04-direction 아닌 contract §2/§8 출처로 동일 커밋 정정] + codex `019e6fcf` round-2 **BLOCKING 0/NEEDS_CHANGES 3/NOTABLE 3**[NC=AC-2 서명 strip/re-sign 과대주장 정정 + duplicate id fail-closed reject + 의미 검증 MUST 신설 → 동일 커밋 흡수]. **KI 변동 0** (잔여 9 유지 — 후속 추적: notion-export-spec 본문 일부 구버전 `v05` 표기 drift). 단위 회귀 변동 0 (코드 미변경, 1947 유지). G-022 자기 적용 — 사용자 entry 발화("핸드오프 읽고 작업 진행해") 후 진입. **직전 (PR #264 T17e E2E 격리, `70d5371`, T17 라인 종결)**: ws1=openai:1024/ws2=ollama:768 write/query 격리 통합 테스트, evaluator `a67b8734cc2b067d8` 6/0/0 + codex 019e6ea0 0/2→0/1, 단위 1942→1947/104 파일. 상세 = handoff 이어쓰기 4.

<!-- 직전 T17e 상세 (보존): PR #264 T17e 임베딩 provider/dimension 격리 E2E 통합 테스트 머지 `70d5371` — CI 6/6 SUCCESS. write-path wiring(#262)으로 닫힌 T17 라인의 end-to-end 가치 증명. 신규 `tests/integration/t17e-embedding-isolation.test.ts` 5 테스트: ws1=openai:1024 / ws2=ollama:768 두 워크스페이스에서 write path(recordVisit→enqueue→processNextEmbeddingJob[buildEmbeddingClientForModel]→ws1 vec_pages_1024/ws2 vec_pages_768 searchPages per-dim) + query path(handleSearchQuery provider 선택+dim 해소→자기 page 만) + provider 선택 격리 spy(1024/text-embedding-3-small vs 768/nomic-embed-text) + 워크스페이스 격리 양방향 + dimension 안전(wrong dim→failed, vector 0). +325/-0/1 파일. dual review evaluator **Pass 6/Partial 0/Fail 0**[agentId `a67b8734cc2b067d8`, 거짓 통과 우려를 **뮤테이션 테스트 2종**(workspace 필터·dim passthrough 무력화 시 단언 fail)으로 반증] + codex `019e6ea0` **BLOCKING 0/NEEDS_CHANGES 2→0/NOTABLE 1**[NC=drainUntilIdle 무한루프 가드 + ws2 query openai 미호출 단언 → 정정 해소, 동일 커밋 amend]. 자기 검증 루프 **63회차**[write-path 62 → T17e 63]. 단위 회귀 1942 → **1947** (+5 T17e; `npm test` 전체 **104 파일** = CI 정합). **KI 변동 0** (잔여 9 유지). G-022 자기 적용 — write-path 머지 후 PR 머지 경계에서 AskUserQuestion → 사용자 명시 선택 "T17e" 로 진입. **직전 (PR #262 write-path wiring `81bcb65`, 2026-05-28)**: enqueue 된 임베딩 잡 처리 prod worker(EmbeddingWorker) + 워크스페이스별 client 해소(buildEmbeddingClientForModel) + provider 미가용 ≠ 영구 실패(release+backoff). evaluator `a0b53b81ac428e1d9` 6/0/0 + codex 019e6ea0 0/0/1, 루프 62, 단위 1912→1942/103 파일. 상세 = handoff 이어쓰기 3. -->
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 3 작업, M2 종료 → M3 진입 / codex 019e6e4d 순차 우선순위):
  1. **T22/T09** (S018-T08~T09, M3) — 단위 테스트 회귀 셋 + 신규 컴포넌트 cover (목표 +80~120) + 시나리오 1·2·3·4 cover 90%+ 검증. codex 019e6e4d 순차상 T21 다음.
  2. **PRD v0.5.0 발행 + Phase 3 종료 검토 + Sprint 019 시안** (S018-T10~T13, M4~M5). **T20 = Sprint 019 defer** (학습 데이터셋 부재, contract §155).
  - ⚠️ 다음 작업(T22/T09) 은 **사용자 명시 선택** 후 진입 (G-022). 본 진입점 박힘 ≠ 본 세션 진입 박음 권고.
  - **후속 (scope 외, MVP 의도 한계)**: [write-path] (a) provider 미가용 고우선 잡 starvation (b) in-flight embed 비취소 (c) query path 인라인 체인 중복(G-013 의도적 비대칭). [T21 Sprint 021 구현 분해 S021-a~k = `.flowset/specs/v05-collab-spike.md` §11]. [drift] notion-export-spec 본문 일부 구버전 `schemaVersion: v05` 표기 (현 코드 SSOT = v06) — T19 spec 별도 정정 후보.
  - (완료: **T17 라인 종결** — T17a~e[#254~#264] + write-path[#262] / **T21 워크스페이스 공유 설계 시안[#266 — SharedWorkspaceFormat]** / T19 Notion Export spec[Sprint 020] / Schema v06 spec[#252])
