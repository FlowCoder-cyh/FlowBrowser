...
- **Phase**: **3 진행 중 (Sprint 018 M2 T17e 머지 완료 `70d5371` — T17 라인 완전 종결, 본 state 동기화 docs PR 진행 중)** (**M0+M1 종료, M2 진행 — T17a 마이그레이션 + T17b wiring + T17c provider 선택 + T17d UX 모델 선택 + write-path wiring + T17e E2E 격리 머지**). carryover **4 작업 잔여** (~~T17 CO-1 종결~~ + T20 + T21 + T22~T26 + PRD v0.5.0). **codex 019e6e4d 우선순위 협의 — 순차 실행: ~~write-path wiring(#262)~~ → ~~T17e(#264)~~ → T21 → T22/T09 → PRD v0.5.0. T20 = Sprint 019 defer(학습 데이터셋 부재).**
- **Sprint**: **018 M2 T17e 머지 완료 (#264 `70d5371`) — T17 라인(로컬 임베딩 통합) 완전 종결, 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **19** (#246~#264 feat/docs, 본 docs PR 머지 시 20 — `git log --first-parent main --grep=WI-S018` 실측). M0+M1 종료 + **M2 진행(T17a~e + write-path wiring 전부 머지)**. 다음 = **T21** (워크스페이스 공유 설계 시안 — 독립 spec). **query path + 생성 UX + write-path 전부 provider-aware + E2E 격리 검증 완료** (OpenAI 1024 ↔ Ollama 768 write/query 격리 통합 테스트).
...
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 4 작업, M2 진행 중 / codex 019e6e4d 순차 우선순위):
  ...
  - (완료: **T17 라인 종결** — T17a Schema v06 마이그레이션[#254] / T17b wiring[#256] / T17c provider 선택[#258] / T17d UX 모델 선택[#260] / write-path wiring[#262 — worker drainer + provider-aware resolveEmbeddingClient] / **T17e E2E 격리 검증[#264 — OpenAI 1024↔Ollama 768 write/query 격리 통합 테스트]** / T19 Notion Export spec[Sprint 020 구현] / Schema v06 spec 복원[#252])
