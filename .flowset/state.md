...
- **Phase**: **3 진행 중 (Sprint 018 M2 T17a 머지 완료 `c2bec41` — 본 state 동기화 docs PR 진행 중)** (**M0+M1 종료, M2 진입 — T17a Schema v06 마이그레이션**). carryover **5 작업 잔여** (T17[CO-1 — T17a 머지 / T17b~e 잔여] + T20 + T21 + T22~T26 + PRD v0.5.0).
- **Sprint**: **018 M2 T17a 머지 완료 (#254 `c2bec41`) — 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **8** (#246~#254 feat/docs, 본 docs PR 머지 시 9). M0+M1 종료 + **M2 진입(T17a)**. 다음 = **T17b** (VectorIndex `vec_pages_1024` 전환 + dimension 분기[upsert=`EmbeddingClient.processNextEmbeddingJob` / query=`searchHandlers.ts:134`, IndexingService/SearchService 클래스 직접 아님 — spec §5] + services.ts 마이그레이션 체인 wiring).
...
- **최근 갱신 (Sprint 018 M2 T17a 머지 완료)**: 2026-05-27 (PR #254 Schema v06 마이그레이션 머지 `c2bec41` — CI 6/6 SUCCESS. T17 로컬 임베딩 통합 진입 — Schema v06 spec §3~§4 기준 v05→v06 마이그레이션. 진입 게이트 PoC 2종 통과[sqlite-vec 0.1.9 float[768] vec0 신규 가능 / vec0 ALTER RENAME 불가→INSERT…SELECT copy / ALTER ADD COLUMN / 변환 transaction commit·rollback 안전]. 산출물: `v06.sql` canonical(+260) + `migrateV05ToV06`(+333, explicit 변환 단일 transaction 래핑 + G-014 dry-run/백업/sentinel) + 회귀 13(+503). `V06_SCHEMA_VERSION=3`. **scope = schema+migration+회귀만, services.ts/VectorIndex wiring 은 T17b**(G-013 단계별 — 앱 v05 유지, 순수 additive). dual review evaluator **Pass 9/Partial 1/Fail 0** + codex `019e658a` **3 라운드 → 0/0/0**[설계 협의 `019e6574` 후보 A]. hotfix 2건 흡수[`ab45073` sentinel atomic+idempotency hard-fail+트리거 회귀 / `e961e23` hard-fail 흔적 전수+CHECK drift 검출]. 자기 검증 루프 **58회차** — 정의: dual review 통과 PR 누적, 직전 +1 [T04 57 → T17a 58]). 직전 (PR #252 Schema v06 spec 복원 `fe60e8d` — evaluator 4/0/0 + codex 019e653c 0/0/0, 57회차). 단위 회귀 1840 → **1853** (+13 마이그레이션 회귀; `npm test` 전체 **100 파일** = CI 정합). **KI 변동 0** (잔여 9 유지). G-022 자기 적용 — 진입 발화 "핸드오프 읽고 작업 진행해" (ALLOW_ENTRY) + AskUserQuestion 으로 M2 작업 선택(T17) 후 진입.
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 5 작업, M2 진행 중):
  1. **T17b 로컬 임베딩 wiring** (S018-T05, M2) — Schema v06 spec §5 기준. services.ts 마이그레이션 체인(`migrateV04ToV05 → migrateV05ToV06`) + VectorIndex `vec_pages`→`vec_pages_1024` 전환 + dimension 분기(EmbeddingClient.processNextEmbeddingJob upsert / searchHandlers.ts:134 query) + SearchService allowlist helper + freshInstall 백업 skip 계약 테스트. 이후 T17c(OllamaProvider.embed) / T17d(UX 모달) / T17e(E2E).
  2. **T20 / T21** — R3-B 실 학습(학습 데이터셋) / 워크스페이스 공유 설계 (S018-T06~T07, M2)
  3. **PRD v0.5.0 + Phase 3 종료 검토 + Sprint 019 시안** (S018-T10~T13, M4~M5)
  - (완료: T17a Schema v06 마이그레이션[#254] / T19 Notion Export = T03 옵션 C spec[구현 Sprint 020] / Schema v06 spec = T04 복원[#252])
