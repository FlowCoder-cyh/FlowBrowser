...
- **Phase**: **3 진행 중 (Sprint 018 M2 T17c 머지 완료 `af6d18e` — 본 state 동기화 docs PR 진행 중)** (**M0+M1 종료, M2 진행 — T17a 마이그레이션 + T17b wiring + T17c 로컬 임베딩 provider 선택 머지**). carryover **5 작업 잔여** (T17[CO-1 — T17a+T17b+T17c 머지 / T17d~e 잔여] + T20 + T21 + T22~T26 + PRD v0.5.0).
- **Sprint**: **018 M2 T17c 머지 완료 (#258 `af6d18e`) — 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **13** (#246~#258 feat/docs, 본 docs PR 머지 시 14 — `git log --first-parent main --grep=WI-S018` 실측). M0+M1 종료 + **M2 진행(T17a+T17b+T17c 머지)**. 다음 = **T17d** (UX 워크스페이스 생성 모달 모델 선택 + CreateWorkspaceInput.embedding_model 개방) / **T17e** (E2E OpenAI↔Ollama 격리 검증). **query path 는 T17c 로 provider-aware** (embedding_model.provider 별 어댑터 선택) — 단 **write-path(`processNextEmbeddingJob`)는 prod worker 호출자 자체가 미연결**(src 내 prod caller 부재, codex 019e6e2a) → 연결 시 `resolveEmbeddingClient` provider 선택까지 붙여야 ollama 실 적재 가능. 실 indexing end-to-end 는 worker caller wiring(scope 外 후속) + T17e 검증까지 필요. 또는 T20/T21.
...
- **최근 갱신 (Sprint 018 M2 T17c 머지 완료)**: 2026-05-28 (PR #258 로컬 임베딩 provider wiring 머지 `af6d18e` — CI 6/6 SUCCESS. Schema v06 spec §5.2~§5.3 기준. T17b 가 박은 dimension/threading 인프라 위에서 로컬 임베딩을 query path 에 wiring. **설계 사전 협의 codex `019e6e00` 후보 A**[query path provider 선택 + namespace→credential 매핑 helper + OllamaProvider.embed, write-path 팩토리/worker 는 caller 부재로 defer=dead code 회피]. 산출물: `OllamaProvider.embed()`[POST /api/embed batch + nomic-embed-text 768 + supportsEmbed=true / request.dimensions 무시=nomic 768 고정, length 검증은 EmbeddingClient.toFloat32 위임 / error 매핑 chat 패턴=network·404 pull 안내·429·5xx·4xx·count mismatch] + `embeddingModel.ts`[embeddingProviderToCredentialProvider() — namespace('openai'|'ollama')→credential('openai'|'local') `as const satisfies Record` 컴파일 타임 누락 차단, registry↔CHECK↔vec0↔credential 4자 일치] + searchHandlers[getEmbeddingProvider(credentialType) 시그니처 + **modelSpec 선해소 reorder**(임베딩 _전_ provider/dim 해소) + provider-aware 에러 + `!info.supportsEmbed||!embed` 능력 가드] + services.ts[providers.get(providerType) wiring, 이전 'openai' 고정 제거]. +428/-70/10 파일. dual review evaluator **Pass 7/Partial 0/Fail 0**[agentId `aeee52271ee56b98b`, AC1~AC7 전부 Pass] + codex `019e6e00` **BLOCKING 0/NEEDS_CHANGES 0/NOTABLE 5**[Q1~Q4 설계 확인 + 능력 가드 hardening 1건→hotfix `9b0d952` 흡수]. 자기 검증 루프 **60회차** — 정의: dual review 통과 PR 누적, 직전 +1 [T17b 59 → T17c 60]). 직전 (PR #256 로컬 임베딩 wiring `4c2ed6e` — evaluator 9/1/0 + codex 019e6898 0/0/4, 59회차). 단위 회귀 1878 → **1894** (+16 OllamaProvider embed 11종 + searchHandlers provider 선택/능력 가드 + embeddingModel 매핑; `npm test` 전체 **102 파일** = CI 정합). **KI 변동 0** (잔여 9 유지). G-022 자기 적용 — 진입 발화 "핸드오프 읽고 작업 진행해" (ALLOW_ENTRY) + AskUserQuestion 으로 M2 작업 선택(T17c) 후 진입.
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 5 작업, M2 진행 중):
  1. **T17d UX 모델 선택 + T17e E2E** (S018-T05 잔여, M2) — T17d(워크스페이스 생성 모달 모델 선택 + `CreateWorkspaceInput.embedding_model` 개방, G-013 UX 단계) / T17e(E2E OpenAI↔Ollama 격리 검증). T17e 전에 **write-path provider 선택 비대칭 해소 필요** — query path 만 provider-aware, write-path(`processNextEmbeddingJob`)는 prod worker 호출자가 아직 미연결이라 연결 시 worker caller + resolveEmbeddingClient provider 선택을 함께 박아야 함(현재 어느 provider 로도 실 적재 안 됨).
  2. **T20 / T21** — R3-B 실 학습(학습 데이터셋) / 워크스페이스 공유 설계 (S018-T06~T07, M2)
  3. **PRD v0.5.0 + Phase 3 종료 검토 + Sprint 019 시안** (S018-T10~T13, M4~M5)
  - **후속 (scope 외)**: (a) `processNextEmbeddingJob` 백그라운드 worker 호출자 wiring + `resolveEmbeddingClient` provider 선택(현재 query path 만 Ollama-capable, write path 는 prod worker 호출자 미연결 — codex 019e6e00 Q4 / 019e6e2a) (b) `WorkspaceExportImportService` export schema v05 고정 + embedding_model 미export — T17d 이후 export schema bump
  - (완료: T17a Schema v06 마이그레이션[#254] / T17b 로컬 임베딩 wiring[#256] / **T17c OllamaProvider.embed + provider 선택[#258]** / T19 Notion Export = T03 옵션 C spec[구현 Sprint 020] / Schema v06 spec = T04 복원[#252])
