...
- **Phase**: **3 진행 중 (Sprint 018 M2 T17d 머지 완료 `1814a04` — 본 state 동기화 docs PR 진행 중)** (**M0+M1 종료, M2 진행 — T17a 마이그레이션 + T17b wiring + T17c provider 선택 + T17d UX 모델 선택 머지**). carryover **5 작업 잔여** (T17[CO-1 — T17a~T17d 머지 / T17e + write-path wiring 잔여] + T20 + T21 + T22~T26 + PRD v0.5.0). **본 세션 codex 019e6e4d 우선순위 협의 — 순차 실행: write-path wiring → T17e → T21 → T22/T09 → PRD v0.5.0. T20 = Sprint 019 defer(학습 데이터셋 부재).**
- **Sprint**: **018 M2 T17d 머지 완료 (#260 `1814a04`) — 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **15** (#246~#260 feat/docs, 본 docs PR 머지 시 16 — `git log --first-parent main --grep=WI-S018` 실측). M0+M1 종료 + **M2 진행(T17a~T17d 머지)**. 다음 = **write-path wiring** (`processNextEmbeddingJob` prod worker 호출자 연결 + `resolveEmbeddingClient` provider 선택 — T17e 전제) → **T17e** (E2E OpenAI↔Ollama 격리). **query path + 생성 UX 는 provider-aware** — 단 **write-path(`processNextEmbeddingJob`)는 prod worker 호출자 미연결** (src 내 prod caller 부재) → 연결 + provider 선택 후 ollama 실 적재 가능. 또는 T21(독립 spec).
...
- **최근 갱신 (Sprint 018 M2 T17d 머지 완료)**: 2026-05-28 (PR #260 워크스페이스 임베딩 모델 선택 UX 머지 `1814a04` — CI 6/6 SUCCESS. Schema v06 spec §6.1 UX 기준. 워크스페이스 생성 시 임베딩 모델을 사용자가 선택하도록 Database→Service→handler→preload→renderer 전 레이어 개방 + export/import round-trip 보존. 산출물: Database.createWorkspace[embedding_model 명시 시 isSupportedEmbeddingModel 검증 + 조건부 6-컬럼 INSERT(v06) / 미주입 시 기존 5-컬럼 DEFAULT path(v05/v06 양립)] + WorkspaceService.validateEmbeddingModel[''/null→DEFAULT, 미지원→invalid_embedding_model] + handler/preload SerializedWorkspace.embeddingModel 직렬화 + renderer CreateWorkspaceModal select + presets.EMBEDDING_MODEL_OPTIONS[main SSOT 분리, 단위 테스트 drift 차단] + WorkspaceExportImportService[embedding_model export/import schema-aware 조건부 INSERT(PRAGMA table_info) + schemaVersion v05→v06(accepted [v04,v05,v06]) + v05-한정 highlights 게이트 회귀 `!== 'v04'` 정정]. +453/-44/13 파일. dual review evaluator **Pass 6/Partial 0/Fail 0**[agentId `a9c3d48adc7a61e7d`, AC1~AC6] + codex `019e6e62` **BLOCKING 0/NEEDS_CHANGES 2→0/NOTABLE 5**[NC=export/import round-trip 손실 + CSS 누락 → hotfix `a6e9704` 흡수(embedding_model 보존 + schemaVersion v06 + highlights 게이트 정정 + CSS)]. 자기 검증 루프 **61회차** — 정의: dual review 통과 PR 누적, 직전 +1 [T17c 60 → T17d 61]). 직전 (PR #258 provider 선택 `af6d18e` — evaluator 7/0/0 + codex 019e6e00 0/0/5, 60회차). 단위 회귀 1894 → **1912** (+18 createWorkspace embedding_model + Service 검증 + handler 직렬화 + export/import round-trip + presets SSOT; `npm test` 전체 **102 파일** = CI 정합). **KI 변동 0** (잔여 9 유지). G-022 자기 적용 — 진입 발화 "핸드오프 읽고 작업 진행해"(ALLOW_ENTRY)+AskUserQuestion 으로 T17c 선택 후 진입 → T17c 머지 후 "코덱스랑 협의해서 우선순위 부여해서 순차진행해"(ALLOW_ENTRY + 위임) → codex 019e6e4d 우선순위 협의 후 T17d 진입.
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 5 작업, M2 진행 중 / codex 019e6e4d 순차 우선순위):
  1. **write-path wiring** (S018-T05 잔여, M2) — `processNextEmbeddingJob` prod worker 호출자 연결 + `resolveEmbeddingClient` provider 선택 (embedding_model.provider 별 어댑터). T17e 전제 (현재 query path/생성 UX 만 provider-aware, write-path 는 worker 호출자 미연결).
  2. **T17e** (S018-T05 잔여, M2) — E2E OpenAI↔Ollama 격리 검증 (인덱싱→검색, 1024/768 격리).
  3. **T21** — 워크스페이스 공유 설계 시안 (S018-T07, M2, 독립 spec). **T20 = Sprint 019 defer** (학습 데이터셋 부재, contract §155).
  4. **T22/T09 → PRD v0.5.0 + Phase 3 종료 + Sprint 019 시안** (S018-T08~T13, M3~M5).
  - **후속 (scope 외)**: (없음 — export/import embedding_model 은 T17d 에서 흡수 완료)
  - (완료: T17a Schema v06 마이그레이션[#254] / T17b wiring[#256] / T17c provider 선택[#258] / **T17d UX 모델 선택 + export/import 보존[#260]** / T19 Notion Export spec[Sprint 020 구현] / Schema v06 spec 복원[#252])
