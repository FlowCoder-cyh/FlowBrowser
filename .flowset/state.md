...
- **Sprint**: **018 M2 T17c 머지 완료 (#258 `af6d18e`) — 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **13** (#246~#258 feat/docs, 본 docs PR 머지 시 14 — `git log --first-parent main --grep=WI-S018` 실측). M0+M1 종료 + **M2 진행(T17a+T17b+T17c 머지)**. 다음 = **T17d** (UX 워크스페이스 생성 모달 모델 선택 + CreateWorkspaceInput.embedding_model 개방) / **T17e** (E2E OpenAI↔Ollama 격리 검증). **query path 는 T17c 로 provider-aware** (embedding_model.provider 별 어댑터 선택) — 단 **write-path(`processNextEmbeddingJob`)는 여전히 OpenAI 고정 + worker caller 부재** → 실 indexing end-to-end 는 worker caller wiring(scope 外 후속) + T17e 검증까지 필요. 또는 T20/T21.
...
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 5 작업, M2 진행 중):
  1. **T17d UX 모델 선택 + T17e E2E** (S018-T05 잔여, M2) — T17d(워크스페이스 생성 모달 모델 선택 + `CreateWorkspaceInput.embedding_model` 개방, G-013 UX 단계) / T17e(E2E OpenAI↔Ollama 격리 검증). T17e 전에 **write-path provider 선택 비대칭 해소 필요** — query path 만 provider-aware, write-path(`processNextEmbeddingJob`)는 worker caller + resolveEmbeddingClient provider 선택 후속 PR 까지 OpenAI 고정.
  2. **T20 / T21** — R3-B 실 학습(학습 데이터셋) / 워크스페이스 공유 설계 (S018-T06~T07, M2)
  3. **PRD v0.5.0 + Phase 3 종료 검토 + Sprint 019 시안** (S018-T10~T13, M4~M5)
  - **후속 (scope 외)**: (a) `processNextEmbeddingJob` 백그라운드 worker 호출자 wiring + `resolveEmbeddingClient` provider 선택(현재 query path 만 Ollama-capable, write path 는 OpenAI 고정 — codex 019e6e00 Q4) (b) `WorkspaceExportImportService` export schema v05 고정 + embedding_model 미export — T17d 이후 export schema bump
