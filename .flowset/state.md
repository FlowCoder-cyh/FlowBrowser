...
- **Phase**: **3 진행 중 (Sprint 018 M2 T17b 머지 완료 `4c2ed6e` — 본 state 동기화 docs PR 진행 중)** (**M0+M1 종료, M2 진행 — T17a Schema v06 마이그레이션 + T17b 로컬 임베딩 wiring 머지**). carryover **5 작업 잔여** (T17[CO-1 — T17a+T17b 머지 / T17c~e 잔여] + T20 + T21 + T22~T26 + PRD v0.5.0).
- **Sprint**: **018 M2 T17b 머지 완료 (#256 `4c2ed6e`) — 본 state 동기화 docs PR 진행 중**. 누적 main first-parent S018 PR: **11** (#246~#256 feat/docs, 본 docs PR 머지 시 12 — `git log --first-parent main --grep=WI-S018` 실측). M0+M1 종료 + **M2 진행(T17a 마이그레이션 + T17b wiring 머지)**. 다음 = **T17c** (OllamaProvider.embed() 재구현 + supportsEmbed=true + **provider 선택 wiring**[searchHandlers/services 의 getEmbeddingProvider 고정 openai → embedding_model.provider 분기 + processNextEmbeddingJob resolveEmbeddingClient 호출자]). T17b 가 modelHint/dimensions threading + 768 테이블 인프라 완비 → T17c provider wiring 후 query path/upsert helper 가 Ollama-capable. **실 indexing end-to-end 는 worker caller wiring + T17e 검증까지 필요** (단순 OllamaProvider.embed() 만으로 부족 — codex 019e6d06). 또는 T20/T21.
...
- **최근 갱신 (Sprint 018 M2 T17b 머지 완료)**: 2026-05-28 (PR #256 로컬 임베딩 wiring 머지 `4c2ed6e` — CI 6/6 SUCCESS. Schema v06 spec §5 기준. T17a(schema+migration) 머지 후 앱이 v06 위에서 부팅하도록 services 체인 + VectorIndex dimension 분기 적용. 산출물: services.ts 체인[`migrateV04ToV05 → migrateV05ToV06({freshInstall: v05.status}) → ensureDefaultWorkspace`] + `embeddingModel.ts`[신규 SSOT — parse/resolve, v06.sql CHECK↔vec0↔UX 3자 일치] + VectorIndex `_1024`/`_768` 분기[selectVecPagesTable/selectVecNotesTable closed allowlist=injection 차단 / upsert·search=dim 파라미터 / has·delete·count=dimension-agnostic 양 테이블=IndexingService 무변경 / upsert cross-dim cleanup] + 소비처 wiring[**dim 을 임베딩 _전_에 해소** — upsert=EmbeddingClient.processNextEmbeddingJob ProcessJobDeps.resolveEmbeddingClient 팩토리 / query=searchHandlers getWorkspaceEmbeddingModel→resolveEmbeddingModel→EmbeddingClient modelHint/dimensions+SearchService.dimensions] + Database WorkspaceRow.embedding_model + 접근자 SELECT * + DEFAULT 방어[v05/v06 양립]. provider 어댑터 매핑(Ollama)=T17c / UX=T17d 위임(G-013). +1192/-305/24 파일. **설계 사전 협의 codex `019e6898` 후보 A**[caller dim 해소 / test-only v06 helper / createWorkspace DEFAULT 유지]. dual review evaluator round-2 **Pass 9/Partial 1/Fail 0**[Partial=cross-dim 전환 직접 회귀 → 최종 HEAD 회귀 추가로 해소] + codex `019e6898` **BLOCKING 1→0 / NEEDS_CHANGES 2→0 / NOTABLE 4**[BLOCKING=dim 해소가 임베딩 _후_라 768 경로 cosmetic → resolve-then-embed 정정 / NC=upsert cross-dim cleanup + searchHandlers 768 query 회귀]. 자기 검증 루프 **59회차** — 정의: dual review 통과 PR 누적, 직전 +1 [T17a 58 → T17b 59]). 직전 (PR #254 Schema v06 마이그레이션 `c2bec41` — evaluator 9/1/0 + codex 019e658a 3라운드→0/0/0, 58회차). 단위 회귀 1853 → **1878** (+25 dimension 분기/부트 체인/embeddingModel 회귀; `npm test` 전체 **102 파일** = CI 정합). **KI 변동 0** (잔여 9 유지). G-022 자기 적용 — 진입 발화 "핸드오프 읽고 작업 진행해" (ALLOW_ENTRY) + AskUserQuestion 으로 M2 작업 선택(T17b) 후 진입.
- **다음 세션 진입점** (본 state 동기화 docs PR 머지 후 — Sprint 018 carryover 5 작업, M2 진행 중):
  1. **T17c OllamaProvider.embed() 재구현 + provider 선택 wiring** (S018-T05, M2) — `/api/embed` 호출 + 768-dim 매핑 + supportsEmbed=true. **+ provider 선택 wiring** (현재 searchHandlers `getEmbeddingProvider()` no-arg + services.ts `providers.get('openai')` 고정 / processNextEmbeddingJob 백그라운드 worker 호출자 부재 — codex 019e6d06). T17b 가 modelHint/dimensions threading + dimension 별 vec0 테이블 인프라 완비 → T17c 에서 embedding_model.provider 별 어댑터 선택 + resolveEmbeddingClient 호출자를 붙이면 query path/upsert helper 가 Ollama-capable. 실 indexing end-to-end 활성은 worker caller wiring + T17e 검증까지 필요. 이후 T17d(UX 워크스페이스 생성 모달 모델 선택 + CreateWorkspaceInput.embedding_model 개방) / T17e(E2E OpenAI↔Ollama 격리).
  2. **T20 / T21** — R3-B 실 학습(학습 데이터셋) / 워크스페이스 공유 설계 (S018-T06~T07, M2)
  3. **PRD v0.5.0 + Phase 3 종료 검토 + Sprint 019 시안** (S018-T10~T13, M4~M5)
  - **후속 (scope 외)**: (a) `processNextEmbeddingJob` 백그라운드 worker 호출자 wiring(현재 분기 능력만, 실 인덱싱 worker 미구현) (b) `WorkspaceExportImportService` export schema v05 고정 + embedding_model 미export — T17d 이후 export schema bump
  - (완료: T17a Schema v06 마이그레이션[#254] / **T17b 로컬 임베딩 wiring[#256]** / T19 Notion Export = T03 옵션 C spec[구현 Sprint 020] / Schema v06 spec = T04 복원[#252])
