...
- ThumbnailStore (v0.3.10 Sprint 012 M1 신규): 메모리 LRU 50 항목, set/get/remove/clear, get도 last-touched 갱신 (true LRU). dataUrl + capturedAt + width + height. main의 setActiveTabView 진입 시점 browserView 변수가 이전 활성 view를 가리킨다는 점을 활용해 captureTabThumbnail(prevTabId) fire-and-forget. capturePage → resize({width:300}) → toDataURL. capture 실패 silent. destroyTabView 시 자동 remove + mainWindow.closed 시 clear.
- Application Menu (v0.3.10 Sprint 012 M3): installApplicationMenu에서 탭 서브메뉴 4 항목 (Ctrl+T / Ctrl+W / Ctrl+Tab / Ctrl+Shift+Tab), mainWindow focus 시점부터 작동.
