...
M3-4 ✅ NoteStore + AiChatHistoryStore + TagStore + EmbeddingQueue (4 storage 모듈, PR #N) — (1) `src/storage/NoteStore.ts` — Note CRUD + 3중 anchor (page/visit nullable + workspace 필수) + ai_tags JSON 라운드트립 + created_by 'user'/'migration' / (2) `src/storage/AiChatHistoryStore.ts` — AiChat CRUD + role 4종 + status 4종 (ok/pending/failed/aborted) + retrieved_items + chat_meta JSON 라운드트립 + updateStatus (pending→ok/failed/aborted) / (3) `src/storage/TagStore.ts` — Tag ensure (idempotent) + 6 kind + UNIQUE (workspace, kind, name) + ai_generated boolean 라운드트립 + PageTag/NoteTag M:N attach/detach (INSERT OR IGNORE) + CASCADE 정합 / (4) `src/storage/EmbeddingQueue.ts` — 백그라운드 큐 + enqueue + claimNext (priority DESC + created_at ASC FIFO + 상태 전환 단일 TX) + markSucceeded/markFailed (attempts +1 + last_error) + cancel (pending/failed만) + purgeSucceeded + workspace CASCADE / (5) `schema/v04.sql` embedding_queue 테이블 신규 + 2 인덱스 (status+priority+created_at / target_type+target_id) — Database.test 11 entity + 11 인덱스 정합 갱신 / (6) PRD §04 entity 7 → 7 + queue 1 추가 의미 (queue 는 운영 데이터). +46 단위 (486 → 532) — Note 10 + Chat 12 + Tag 12 + Queue 12
