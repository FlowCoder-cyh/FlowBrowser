...
M2-8 TranslationCache.test → AIResponseCache.test 재작성 + PageResultStore.test → IndexedPageStore.test 재작성 + services.ts dead export cleanup (`extractWebContentsPageNodes` / `pageResultLookup` / `persistPageResult` / `nodesSignatureFromTexts` — M2-5/M2-6 폐기 후 호출지점 0, codex M2-6 권고 A+C)
```
