...
M3-2 ✅ VectorIndex 모듈 + vec_pages/vec_notes cascade trigger (PR #133) — (1) `src/storage/VectorIndex.ts` 신규 — sqlite-vec wrapper 클래스 + upsertPage/NoteEmbedding (DELETE+INSERT atomic 단일 TX, sqlite-vec native UPSERT 미지원) + searchPages/Notes (workspace partition 격리 강제) + deleteByPage/Note/Workspace + countPages/Notes + EMBEDDING_DIMENSIONS=1024 + embeddingToBuffer/bufferToEmbedding 라운드트립 + 차원 검증 / (2) `src/storage/schema/v04.sql` 트리거 2종 추가 — `pages_after_delete_vec_pages` + `notes_after_delete_vec_notes` (외래키 virtual table 미지원 → AFTER DELETE trigger 로 PRD §04.6 cascade 보완) / (3) m3-spike-decisions §3.4 인터페이스 정합 — workspace_id 격리 + cosine distance ASC + k 양수 정수 검증. in-memory fallback 미발동 (PoC 통과로 sqlite-vec 단일 경로 박힘). +19 단위 테스트 (442 → 461 — helpers 3 + CRUD 7 + search 5 + cascade trigger 3 + UPSERT 시멘틱 + dim mismatch + k 검증)
M3-3 ✅ IndexedPageStoreSqlite 신규 — JSON IndexedPageStore 와 별개 SQLite backend 클래스 (PR #134) — (1) `src/storage/IndexedPageStoreSqlite.ts` 신규 — FlowbrowserDatabase pages/visits 테이블 활용. 동일 public API + better-sqlite3 단일 TX recordVisit (PRD §05.4.1 정합). prepared statement 캐시 + DEFAULT_WORKSPACE_ID 'default' string → 실 Workspace UUID 자동 매핑 (defaultWorkspaceId constructor 옵션 필수) / (2) JSON IndexedPageStore 보존 (M5-8 일괄 제거 contract) — M3-6 마이그레이션 시점에 데이터 이전 / (3) URL 정규화 + content_hash dedupe + 재방문 시 visited_count++ + content 변경 감지 + 워크스페이스 격리 (같은 URL 다른 워크스페이스 → 별도 page). +25 단위 (461 → 486) — construction 2 + upsertPage 6 (created/unchanged/updated_changed/매핑/URL 정규화/빈 url throw) + recordVisit 4 (TX/재방문/본문 변경/dwell·timestamp) + createVisit 3 (없는 page throw / workspace mismatch throw / listVisits 시간순) + lookup/counts/stats 4 + delete 3 + workspace 격리 2 + clearAll/flush
M3-4 ✅ NoteStore + AiChatHistoryStore + TagStore + EmbeddingQueue (4 storage 모듈, PR #135) — (1) `src/storage/NoteStore.ts` — Note CRUD + 3중 anchor (page/visit nullable + workspace 필수) + ai_tags JSON 라운드트립 + created_by 'user'/'migration' / (2) `src/storage/AiChatHistoryStore.ts` — AiChat CRUD + role 4종 + status 4종 (ok/pending/failed/aborted) + retrieved_items + chat_meta JSON 라운드트립 + updateStatus (pending→ok/failed/aborted) / (3) `src/storage/TagStore.ts` — Tag ensure (idempotent) + 6 kind + UNIQUE (workspace, kind, name) + ai_generated boolean 라운드트립 + PageTag/NoteTag M:N attach/detach (INSERT OR IGNORE) + CASCADE 정합 / (4) `src/storage/EmbeddingQueue.ts` — 백그라운드 큐 + enqueue + claimNext (priority DESC + created_at ASC FIFO + 상태 전환 단일 TX) + markSucceeded/markFailed (attempts +1 + last_error) + cancel (pending/failed만) + purgeSucceeded + workspace CASCADE / (5) `schema/v04.sql` embedding_queue 테이블 신규 + 2 인덱스 (status+priority+created_at / target_type+target_id) — Database.test 11 entity + 11 인덱스 정합 갱신 / (6) PRD §04 entity 7 → 7 + queue 1 추가 의미 (queue 는 운영 데이터). +46 단위 (486 → 532) — Note 10 + Chat 12 + Tag 12 + Queue 12
M3-5 ✅ EmbeddingClient + processNextEmbeddingJob 파이프라인 (PR #136) — (1) `src/ai/embedding/EmbeddingClient.ts` 신규 — ProviderAdapter.embed() wrapper (M2-7 활용) + text-embedding-3-small 1024 차원 디폴트 (BYOK 디폴트, Codex OAuth 임베딩 미지원) + embedTexts batch / embedText 단일 / embedPage (content 우선, title fallback) / embedNote (selected_text + body 연결) + 차원 검증 + provider.embed 미지원 시 throw / (2) processNextEmbeddingJob 헬퍼 — Queue.claimNext → target 본문 조회 (Page/Note) → embed → VectorIndex upsert → markSucceeded / target 미존재 시 markFailed('orphan') / embed 실패 시 markFailed (status 'failed') / 빈 큐 'idle' / (3) `src/ai/index.ts` export 추가. +18 단위 (532 → 550) — EmbeddingClient 12 (embed 미지원 throw / batch / 단일 / 빈 입력 throw / 차원 mismatch / count mismatch / custom dims / embedPage content+title fallback+빈 throw / embedNote sel+body / sel 단독) + processNextEmbeddingJob 6 (idle / page 성공 / note 성공 / orphan / embed 실패 / priority FIFO 정합)
M3-6 ✅ migrations/v03_to_v04.ts + dry-run + revert + 8 회귀 케이스 (G-014 활성, PR #137) — (1) `src/storage/migrations/v03_to_v04.ts` 신규 — A3 §B 5단계 절차 구현 (sentinel/source 검사 → 자동 백업 `<userDataDir>/backup/v03/<ISO_ts>/` → 매핑 5종 → 원본 .deprecated 접미사 → 로그) / (2) 매핑 5종 — Glossary→Note (ai_tags=['glossary', 'domain:xx']) / PageResults→Page+Visit (workspace_id 부여) / TranslationCache→AIResponseCache (kind:translation, summary skip) / UserSetting (translationMode/cancelOnTabSwitch 제거 + workspaceDefault/userLevelPreference/shortcutOverride/privacyExclusions 추가) / TabState (workspaceId 부여, 기존 보존) / (3) revert 절차 — 오류 시 백업 복원 + .deprecated 정리 + sentinel + ai-response-cache.json 삭제 (idempotent 재시도) / (4) sentinel `flowbrowser.db` 파일 — in-memory mode 멱등성 보장 (이미 존재 시 already_migrated) / (5) G-014 가드레일 활성 — dry-run + 자동 백업 + revert + revert 후 sentinel 미생성 보장. +10 단위 (550 → 560) — 8 회귀 케이스 (빈 데이터 / Glossary 3→Note 3 / TranslationCache summary skip / PageResults workspace_id / UserSetting 폐기 키 제거 / TabState 보존 + 부여 / 오류 revert / idempotent) + 2 추가 (backup+log+sentinel 산출 + embeddingQueue 조건부 enqueue)
M3-7 ✅ UsageLog GENERALIZE schema 마이그레이션 + PageCachePanel PARTIAL (PR #N) — (1) `src/storage/UsageLog.ts` 확장 — V04Feature 신규 4종 (chat/embed/tag/background_translation) + V03Feature 보존 (감사 로그 보존성) + V04_FEATURES 상수 export / (2) 옵션 컬럼 3종 추가 — workspaceId / model / durationMs (v0.4 워크스페이스별 비용 + 모델별 / 지연 추적) / (3) summarize() 확장 — byWorkspace + byModel 집계 추가 ('unknown' bucket fallback) / (4) migrateLegacyEntries(defaultWorkspaceId) helper — v0.3 entry 에 workspaceId 부여 (idempotent, 이미 있는 entry 보존) / (5) `src/privacy/types.ts` TransmissionLogEntry.feature → TransmissionFeature (9종 정합) / (6) `src/renderer/src/settings/PageCachePanel.tsx` PARTIAL — "페이지 번역 결과" → "페이지 본문 캐시" copy 갱신 (M5-8 시점에 MemoryStatsPanel 로 흡수 폐기). +7 단위 (560 → 567) — V04_FEATURES 상수 / v0.4 entry 영속 / summarize byWorkspace+byModel / unknown bucket / migrateLegacyEntries idempotent / 빈 파일 0건 / feature enum v0.3+v0.4 9종 모두 허용
M3-4 NoteStore / AiChatHistoryStore / TagStore / EmbeddingQueue
