 ...
 *
 * 정책:
 *   - `allowedProviders` 가 명시되고 **빈 배열이 아니면** 그대로 사용 (기존 BYOK 정합 유지 — KI-003)
 *   - 미명시 또는 빈 배열 시 `defaultProviderId === 'local'` 이면 `['local']` 단일
 *   - 그 외 (`'openai'` / 그 외 알 수 없는 string 포함) 디폴트 `['openai']` — KI-003 BYOK 디폴트 보존
 *
 * codex 019e503a NEEDS_CHANGES 정합 — 빈 배열 = "미명시" 동의 (caller 가 명시적으로 모든 provider
 * 금지하려면 호출 자체 안 함). docstring 과 runtime 의미 일치 (length > 0 검사).
 *
